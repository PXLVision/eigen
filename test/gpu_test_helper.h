...
void run_serialized(index_sequence<Indices...>, index_sequence<OutputIndices...>,
  ...
  buff_ptr = Eigen::deserialize(buff_ptr, Eigen::tuple_get<Indices>(args)...);
  
  // Call function, with void->Void conversion so we are guaranteed a complete
  // output type.
  auto result = void_helper::call(kernel, Eigen::tuple_get<Indices>(args)...);
  
  // Determine required output size.
  size_t output_size = sizeof(size_t);
  output_size += Eigen::serialize_size(Eigen::tuple_get<OutputIndices>(args)...);
