...
void run_serialized(index_sequence<Indices...>, index_sequence<OutputIndices...>,
  ...
  Eigen::tuple_impl::tuple<typename std::decay<Args>::type...> args = Eigen::tuple_impl::make_tuple(typename std::decay<Args>::type{}...);
  ...
  buff_ptr = Eigen::deserialize(buff_ptr, Eigen::tuple_impl::get<Indices, typename std::decay<Args>::type...>(args)...);
  
  // Call function, with void->Void conversion so we are guaranteed a complete
  // output type.
  auto result = void_helper::call(kernel, Eigen::tuple_impl::get<Indices, typename std::decay<Args>::type...>(args)...);
  
  // Determine required output size.
  size_t output_size = sizeof(size_t);
  output_size += Eigen::serialize_size(Eigen::tuple_impl::get<OutputIndices, typename std::decay<Args>::type...>(args)...);
  ...
  if (output_size <= capacity) {
    ...
    buff_ptr = Eigen::serialize(buff_ptr, Eigen::tuple_impl::get<OutputIndices, typename std::decay<Args>::type...>(args)...);
...
auto run_serialized_on_gpu(index_sequence<Indices...>, 
  ...
  // Deserialize outputs.
  auto args_tuple = Eigen::tuple_impl::tie(args...);
  EIGEN_UNUSED_VARIABLE(args_tuple)  // Avoid NVCC compile warning.
  host_ptr = Eigen::deserialize(host_ptr, Eigen::tuple_impl::get<OutputIndices, Args...>(args_tuple)...);
