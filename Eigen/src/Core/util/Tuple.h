...
#ifndef EIGEN_TUPLE_H
#define EIGEN_TUPLE_H

#include <type_traits>
#include <utility>

// Eigen::Tuple is a replacement for std::tuple that can be used in device code.
...
template <typename... Args>
EIGEN_CONSTEXPR EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE
Eigen::Tuple<Args...> forward_as_tuple(Args&&... args) {
  return Eigen::Tuple<Args...>(std::forward<Args>(args)...);
}
