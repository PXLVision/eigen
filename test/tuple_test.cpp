...
void basic_tuple_test() {
  ...
  VERIFY_IS_EQUAL(Eigen::tuple_get<1>(tuple_make), tmp);
  ...
  VERIFY_IS_EQUAL(Eigen::tuple_get<1>(tuple_forward), tmp);
  ...
  VERIFY_IS_EQUAL( (Eigen::tuple_get<1>(Eigen::tuple_get<2>(tuple_tie))), 5.0 );
  // Modify value and ensure tuple2 is updated.
  Eigen::tuple_get<1>(Eigen::tuple_get<2>(tuple_tie)) = 10.0;
  VERIFY_IS_EQUAL( (Eigen::tuple_get<1>(tuple2)), 10.0 );
}

void eigen_tuple_test() {
  Eigen::Tuple<Eigen::Matrix3d, Eigen::MatrixXd> tuple;
  Eigen::tuple_get<0>(tuple).setRandom();
  Eigen::tuple_get<1>(tuple).setRandom(10, 10);
  
  auto tuple_tie = Eigen::tie(Eigen::tuple_get<0>(tuple), Eigen::tuple_get<1>(tuple));
  Eigen::tuple_get<1>(tuple_tie).setIdentity();
  VERIFY(Eigen::tuple_get<1>(tuple).isIdentity());
}
